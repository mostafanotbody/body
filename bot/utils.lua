...
tdcli_function ({
  ...
  if not lang then
    ...
    return tdcli.sendMessage(msg.to.id, msg.id, 0, "`تم ترقية جميع مشرفين المجموعة ✅`", 0, "md")
     end
 end
...
function ban_user(user_name, user_id, chat_id)
    local administration = load_data(_config.moderation.data)
if administration[tostring(chat_id)]['banned'][tostring(user_id)] then
     return
   end
administration[tostring(chat_id)]['banned'][tostring(user_id)] = user_name
    save_data(_config.moderation.data, administration)
 end

function silent_user(user_name, user_id, chat_id)
    local administration = load_data(_config.moderation.data)
if administration[tostring(chat_id)]['is_silent_users'][tostring(user_id)] then
     return
   end
administration[tostring(chat_id)]['is_silent_users'][tostring(user_id)] = user_name
    save_data(_config.moderation.data, administration)
 end

 function unban_user(user_id, chat_id)
    local administration = load_data(_config.moderation.data)
if not administration[tostring(chat_id)]['is_silent_users'][tostring(user_id)] then
    return
   end
administration[tostring(chat_id)]['is_silent_users'][tostring(user_id)] = nil
    save_data(_config.moderation.data, administration)
  end

function unsilent_user(user_id, chat_id)
    local administration = load_data(_config.moderation.data)
if not administration[tostring(chat_id)]['banned'][tostring(user_id)] then
    return
   end
administration[tostring(chat_id)]['banned'][tostring(user_id)] = nil
    save_data(_config.moderation.data, administration)
  end
...
if is_gbanned(user_id) then
  ...
  administration['gban_users'][tostring(user_id)] = user_name
    save_data(_config.moderation.data, administration)
  end
...
if not is_gbanned(user_id) then
  ...
  administration['gban_users'][tostring(user_id)] = nil
    save_data(_config.moderation.data, administration)
  end
...
local lang = redis:get(hash)
  ...
  if next(data[tostring(chat_id)]['banned']) == nil then --fix way
     if not lang then
					...
					return "`قائمة محظورين فارغة`"
              end
				end
       if not lang then
   message = '*List of banned users :*\n'
         else
   message = '_قائمة المحظورين :_\n'
     end
...
local lang = redis:get(hash)
  ...
  if next(data[tostring(chat_id)]['is_silent_users']) == nil then --fix way
   ...
   message = '_قائمة اعضاء المصمتين :_\n'
    end
...
local lang = redis:get(hash)
  ...
  if next(data[tostring(chat_id)]['whitelist']) == nil then --fix way
     if not lang then
					...
					return "`القائمة البيضاء فارغة`"
              end
				end
       if not lang then
   message = '*Users of white list :*\n'
         else
   message = '`اعضاء القائمة البيضاء :`\n'
     end
...
local lang = redis:get(hash)
  ...
  if next(data['gban_users']) == nil then --fix way
    if not lang then
					...
					return "`قائمة الحظر العام فارعة`"
             end
				end
        if not lang then
   message = '*List of globally banned users :*\n'
   else
   message = '`قائمة اعضاء الحظر العام :`\n'
   end
  ...
  if not data[tostring(msg.chat_id_)]['filterlist'] then
    ...
    save_data(_config.moderation.data, data)
      ...
      if not lang then
       ...
       filterlist = '`قائمة الكلمات المحظورة :`\n'
    end
 local i = 1
   for k,v in pairs(data[tostring(msg.chat_id_)]['filterlist']) do
